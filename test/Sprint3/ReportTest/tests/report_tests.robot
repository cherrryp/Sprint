...
TC10 - ส่ง Report สำเร็จ (Happy Path)
    ...
    # ถ้าไม่มีปุ่มเพราะ report ไปแล้วทุก trip → ถือว่า guard ทำงานถูก แต่ mark SKIP ชัดเจน
    Run Keyword If    '${found}' == 'none'
    ...    Skip    ไม่มีปุ่ม report เพราะ trip ทั้งหมดถูก report ไปแล้ว
    ...    (ให้รัน TC16 เพื่อ verify guard แทน)
    Open Report Modal
...
TC12 - คลิก "ประวัติการรีพอร์ต" บน Navbar นำทางไปหน้า myHistory
    [Documentation]    กดลิงก์ "ประวัติการรีพอร์ต" ใน navbar → URL ต้องเปลี่ยนเป็น /myHistory
    [Tags]    report    navigation    guard
    # navbar template มี NuxtLink to="/myHistory" ข้อความ "ประวัติการรีพอร์ต"
    Wait Until Element Is Visible
    ...    xpath=//nav//a[contains(@href,'/myHistory') or contains(.,'ประวัติการรีพอร์ต')]
