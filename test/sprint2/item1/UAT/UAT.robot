...
Logout
    Wait Until Element Is Visible    xpath=//div[contains(@class,'dropdown-trigger')]    10s
...
# ---------------------------
# Keywords สำหรับคำขอข้อมูล (แก้ไขใหม่ให้หาปุ่มจากชื่อไฟล์)
# ---------------------------
Approve Export Request By Format
    [Arguments]    ${format}
    # หาแถวที่มีคำว่า CSV/JSON/PDF แล้วกดปุ่ม "อนุมัติ" ในแถวนั้น
...
Download User File By Format
    [Arguments]    ${format}
    # หาแถวที่มีคำว่า CSV (หรืออื่นๆ) แล้วกดปุ่ม "ดาวน์โหลด" ในแถวนั้น
    ${btn_xpath}=    Set Variable    xpath=//tr[contains(., '${format}')]//button[contains(normalize-space(), 'ดาวน์โหลด')]
    Wait Until Element Is Visible    ${btn_xpath}    10s
    Scroll Element Into View         ${btn_xpath}
    Click Button                     ${btn_xpath}
    
    Wait Until Page Contains    กำลังดาวน์โหลด    10s
...
UAT-PNN-004 User Request Data Log
    ...
    Login As User
    Sleep    3s
    # 1. User goes to profile logs requests directly
    Go To    ${BASE_URL}profile/logs
    Wait Until Location Contains    /profile/logs    10s
    
    # [แก้ไข] ทดสอบการยื่นขอไฟล์แบบ CSV อันเดียว
    Submit User Data Request    CSV
    Sleep    2s
    Logout
    Sleep    2s
    # 2. Admin logs in to approve the requests directly
    Go To Login Page
    Login As Admin And Go Dashboard
    ...
    # [แก้ไข] อนุมัติโดยเจาะจงชื่อ Format CSV อันเดียว
...
# 3. User logs in again to download the files directly
    Go To Login Page
    Login As User
    ...
    # [แก้ไข] กดดาวน์โหลด เช็คข้อความบนจอ แล้วจบเทสต์เลย
